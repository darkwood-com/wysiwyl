...
class SantaControllerTest extends BaseWebTestCase
{
    ...
    public function testFinishPageWorksWithValidHashForSuccessfulWysiwyl(): void
    {
        ...
        $wysiwyl = new Wysiwyl('azerty', [
            'toto1' => 'toto2',
            'toto2' => 'toto3',
        ], $config);
        $wysiwyl->markAssociationAsProceeded('toto1');
        $wysiwyl->markAssociationAsProceeded('toto2');

        $client = static::createClient();
        $this->prepareSession($client, 'wysiwyl-azerty', $wysiwyl);
    ...
    public function testFinishPageWorksWithValidHashForFailedWysiwyl(): void
    {
        $config = new Config('my_application', 'toto', null);
        $config->setAvailableUsers(
            [
        ...
        $wysiwyl->addError('Knock knock. Who\'s there? A santa error!', 'toto1');
