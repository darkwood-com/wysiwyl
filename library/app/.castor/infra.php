...
function destroy(
    ...
    $files = finder()
        ->in(variable('root_dir') . '/tools/env/services/router/certs/')
        ->name('*.pem')
        ->files()
    ;
    fs()->remove($files);
...
function generate_certificates(
    ...
    if (file_exists(variable('root_dir') . '/tools/env/services/router/certs/cert.pem') && !$force) {
    ...
    if ($force) {
        if (file_exists($f = variable('root_dir') . '/tools/env/services/router/certs/cert.pem')) {
            io()->comment('Removing existing certificates in tools/env/services/router/certs/*.pem.');
            unlink($f);
        }

        if (file_exists($f = variable('root_dir') . '/tools/env/services/router/certs/key.pem')) {
            unlink($f);
        }
    }
    ...
    if ($mkcert) {
        ...
        run([
            'mkcert',
            '-cert-file', 'tools/env/services/router/certs/cert.pem',
            '-key-file', 'tools/env/services/router/certs/key.pem',
            $rootDomain,
            "*.{$rootDomain}",
    ...
    run(['tools/env/services/router/generate-ssl.sh'], quiet: true);

    io()->success('Successfully generated self-signed SSL certificates in tools/env/services/router/certs/*.pem.');
