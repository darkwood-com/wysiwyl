...
namespace Darkwood\Wysiwyl\Statistic;
...
class StatisticCollector
{
    ...
    public function incrementUsageCount(Wysiwyl $wysiwyl): void
    {
        $applicationCode = $wysiwyl->getConfig()->getApplication();
        $currentYear = date('Y');
        $currentMonth = date('m');
        ...
        $this->client->incrby('stats:users', $wysiwyl->getUserCount());

        $usersMax = (int) $this->client->get('stats:users-max');
        if ($wysiwyl->getUserCount() > $usersMax) {
            $this->client->set('stats:users-max', $wysiwyl->getUserCount());
        }
    }

    public function incrementSampleCount(Wysiwyl $wysiwyl): void
    {
